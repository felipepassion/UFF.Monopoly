...
public partial class Play : ComponentBase, IAsyncDisposable
{
    ...
    private async Task CloseBlockModal()
    {
        ...
        if (_modalFromMove && _game is not null && _modalPlayer is not null)
        {
            ...
            { await RegisterLoserAsync(_modalPlayer); CleanupModal(); ResetPendingSpecial(); return; }
        ...
        // Defer bot auto roll until dialogue finishes
        if (IsDialogueBusy) { await WaitForDialogueIdleAsync(200, _turnCts?.Token); }
        await TryAutoRollForBotAsync();
    }

    private void CleanupModal() { _modalFromMove = false; _modalTemplateEntity = null; try { _modalCts?.Cancel(); } catch { } }
    ...
    private async Task ShowActionToastAsync(string message, int durationMs = 2000)
    {
        ...
        StateHasChanged();
        try { await Task.Delay(durationMs, _modalCts?.Token ?? CancellationToken.None); } catch { }
